...
const EachBlog = ({post,w="max-w-lg"}) => {
  return (
    <Link href={`/blog/${post.slug}`} className={`w-full ${w} rounded-2xl   overflow-hidden group`}>
    ...
    <Image alt="" width={320} height={240}
          ...
          <div className="flex flex-col gap-2 px-2 mt-6">
            <p className="text-sm font-semibold text-slate-500">Category - {post.categories?  post.categories?.map(category => <span key={category}>{category} &nbsp;</span>) :"Uncategorised" }</p>
