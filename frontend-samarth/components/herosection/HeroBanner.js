...
import { staggerContainer,item,fadeIn } from "../../lib/Animation";
...
const HeroBanner = () => {
  ...
  return (
    <>
        ...
        <m.div variants={staggerContainer} initial="hidden" whileInView="show" viewport={{once:true, amount:0.1}} className="static lg:absolute  -bottom-[250px] left-0 z-[20]  md:flex-row  flex-col px-2  flex w-full [&>div>svg]:text-5xl [&>div]:max-w-xl items-center lg:items-start
...
md:[&>div>h3]:text-2xl  [&>div>h3]:font-semibold [&>div>h3]:text-slate-800 [&>div>p]:text-gray-500 [&>div>p]:font-normal lg:px-12 [&>div>p]:mt-4 text-justify lg:[&>div]:w-1/3">
        ...
        <m.div variants={fadeIn('up','tween',0.4,0.5)}    className={` flex flex-col bg-white  rounded-xl lg:shadow-2xl lg:shadow-teal-100/70 lg:self-stretch group `}>
        ...
        <m.div variants={fadeIn('up','tween',0.6,0.5)}   className={` flex flex-col bg-white rounded-xl lg:shadow-2xl lg:shadow-teal-100/70 lg:self-stretch group `}>
